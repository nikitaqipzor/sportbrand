...
import { nextSetNumber, type ActiveWorkout, type StartWorkoutInput } from "./active-workout.ts";
...
export function useActiveWorkout(input: StartWorkoutInput): ActiveWorkoutView {
  ...
  }, [offline, userId, input.workoutId, input.title, input.exerciseId, sync]);

  const recordSet = useCallback(
    async (measures: SetMeasures): Promise<void> => {
      if (!userId || !workout || busy) return;
      setBusy(true);
      const result = await offline.recordSet(userId, workout, measures);
      if (!result.ok) {
        if (alive.current) {
  ...
  return {
    userId,
    workout,
    setNumber: workout ? nextSetNumber(workout) : 1,
    status,
    queue,
    issues,
    busy,
